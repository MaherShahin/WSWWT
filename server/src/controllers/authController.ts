...
export class AuthController {
  ...
  static register = handleApiResponse(async (req: Request, res: Response, next: NextFunction) => {
  ...
  static login = handleApiResponse(async (req: Request, res: Response, next: NextFunction) => {
    const { email, password } = req.body;
    const response = await AuthService.loginUser(email, password);
    return new ApiResponse('User logged in successfully', response);
  });

  static getMe = handleApiResponse(async (req: Request, res: Response, next: NextFunction) => {
