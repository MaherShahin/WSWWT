...
export class FriendService {
  static async addFriend(userId: Types.ObjectId, friendId: Types.ObjectId) {
    ...
    if (!user || !friend) {
      throw new ValidationError(
        "Could not add friend, either user or friend dont exist",
      );
    }
  ...
  static async removeFriend(userId: Types.ObjectId, friendId: Types.ObjectId) {
    ...
    if (!user || !friend) {
      throw new ValidationError(
        "Could not remove friend, either user or friend dont exist",
      );
    }
  ...
  ): Promise<IFriendRequest | { message: string }[]> {
  ...
  static async acceptFriendRequest(
    ...
    userId: Types.ObjectId,
  ): Promise<IFriendRequest | IUser | null> {
    const request = await FriendRequest.findById(requestId);
    if (
      !request ||
      request.status !== "pending" ||
      !request.to.equals(userId)
    ) {
