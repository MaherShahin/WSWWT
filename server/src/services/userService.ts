...
export class UserService {
  ...
  static async addJoinedRoomToUser(
    userId: Types.ObjectId,
    ...
    if (!user) {
      throw new NotFoundError(
        "User not found, you cant add a joined room to a user that doesnt exist",
      );
    }
  ...
  static async removeJoinedRoomFromUser(
    userId: Types.ObjectId,
    roomId: Types.ObjectId,
  ): Promise<IUser | null> {
    const user = await this.findUserById(userId);
    if (!user) {
      throw new NotFoundError(
        "User not found, you cant remove a joined room from a user that doesnt exist",
      );
    }
  ...
  static async addCreatedRoomToUser(
    userId: Types.ObjectId,
    roomId: Types.ObjectId,
  ) {
    ...
    if (!user) {
      throw new NotFoundError(
        "User not found, you cant add a created room to a user that doesnt exist",
      );
    }
